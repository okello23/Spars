...
class ApiController extends Controller
{
  ...
  public function saveStockManagement(Request $request)
  {
    ...
    if($exists==null)
    {
      ...
      $stock_management->upload_status = 1;
    ...
    if($indicators == null )
    {
      ...
      $indicator_score->upload_status = 1;
  ...
  public function saveLabInfoSystem(Request $request)
  {
    ...
    if($information_system23 == null )
    {
      ...
      $information_system23->upload_status = 1;
    ...
    if($information_system24 == null )
    {
      ...
      $information_system24->upload_status = 1;
  ...
  public function remoteUpload()
  {
    ...
    $surveysummary = SurveySummary::where('step', '=', 6)->where('upload_status', '=', 1)->get();
    ...
    foreach ($surveysummary as $value ) {
      ...
      DB::connection('mysql_remote')->insert('insert into survey_summary (id,visit_date, next_visit_date,health_facility_id, created_at, updated_at,form_id, visit_number, created_by, updated_by,step,form_version) values (?,?, ?,?, ?,?, ?,?, ?,?, ?,?)', [$id,$vd,$nvd,$hf,$ca,$ua,$fi,$vn,$cb,$ub,$st,$fv]);

      DB::table('survey_summary')->where('upload_status','=',1)->update(['upload_status' => 2]);
    }



    $supervised = SupervisedPerson::where('upload_status', '=', 1)->get();
    foreach ($supervised as $value ) {
      ...
      DB::connection('mysql_remote')->insert('insert into spars_supervised (id,health_facility_id,form_id,visit_date, name, gender,cadre_id,phone_number, created_by,updated_by,created_at, updated_at) values (?,?,?, ?,?, ?,?, ?,?, ?,?,?)', [$id,$hf,$fi,$vd,$nm,$gd,$ci,$pn,$cb,$ub,$ca,$ua]);
      DB::table('spars_supervised')->where('upload_status','=',1)->update(['upload_status' => 2]);

    }

    $supervisor = Supervisor::where('upload_status', '=', 1)->get();
    foreach ($supervisor as $value ) {
     ...
     DB::connection('mysql_remote')->insert('insert into spars_supervisors (id,health_facility_id,form_id,visit_date, person_id, created_by,updated_by,created_at, updated_at) values (?,?,?,?,?,?,?,?,?)', [$id,$hf,$fi,$vd,$pi,$cd,$ub,$ca,$ua]);
       DB::table('spars_supervisors')->where('upload_status','=',1)->update(['upload_status' => 2]);

    }

    $healthfacility = HealthFacility::where('upload_status', '=', 1)->get();
    foreach ($healthfacility as $value ) {
     ...
     DB::connection('mysql_remote')->insert('insert into health_facilities (dhis2_uid,region,district, hsd,facility, level, ownership,in_charge_fname, in_charge_lname, in_charge_contact, lss_fname, lss_lname, lss_contact,visit_number, is_control_facility) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [$dhi,$rgn,$dst,$hsd,$fc,$lvl,$own,$ifn,$iln,$ic,$lfn,$lfn,$lct,$vn,$icf]);
       DB::table('health_facilities')->where('upload_status','=',1)->update(['upload_status' => 2]);
    }

    $general = General::where('upload_status', '=', 1)->get();
    foreach ($general as $value ) {
      ...
      $ua = $value->updated_at;
      // $upload_status;
      DB::connection('mysql_remote')->insert('insert into spars_general (id,health_facility_id,form_id,d1,d2a,d2b,d2c,d2d,d2e,d2f,d2comment,d3,d3comment,d4a,d4b,d4c,d4d,d4e,d4f,d4comment,d5,visit_date,created_by,updated_by,created_at,updated_at) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',[$id,$hf,$fi,$d1,$d2a,$d2b,$d2c,$d2d,$d2e,$d2f,$d2c,$d3,$d3ct,$d4a,$d4b,$d4c,$d4d,$d4e,$d4f,$d4ct,$d5,$vd,$cb,$ub,$ca,$ua]);
        DB::table('spars_general')->where('upload_status','=',1)->update(['upload_status' => 2]);
    }

    $stockMgt = StockManagement::where('upload_status', '=', 1)->get();
    ...
    foreach ($scoresummary as $value ) {
      ...
      DB::connection('mysql_remote')->insert('insert into spars_summary_scores(id,health_facility_id,survey_summary_id,form_id,indicator1_score,indicator2_score,indicator3_score,indicator4_score,indicator5_score,indicator6_score,indicator7_score,indicator8_score,indicator9_score,indicator10_score,indicator11_score,indicator12_score,indicator13_score,indicator14_score,indicator15_score,indicator16_score,indicator17_score,indicator18_score,indicator19_score,indicator20_score,indicator21_score,indicator22_score,indicator23_score,indicator24_score,indicator25_score,indicator26_score,indicator27_score,visit_number,visit_date,created_by,updated_by,created_at,updated_at) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [$id,$hfi,$ssi,$fid,$i1,$i2,$i3,$i4,$i5,$i6,$i7,$i8,$i9,$i10,$i11,$i12,$i13,$i14,$i15,$i16,$i17,$i18,$i19,$i20,$i21,$i22,$i23,$i24,$i25,$i26,$i27,$vn,$vd,$cb,$ub,$ca,$ua]);

        DB::table('spars_summary_scores')->where('upload_status','=',1)->update(['upload_status' => 2]);
    }

    $storage10 = StorageManagement10::where('upload_status', '=', 1)->get();
    foreach ($storage10 as $value ) {
      ...
      DB::connection('mysql_remote')->insert('insert into spars_storage_management_10 (id,health_facility_id,survey_summary_id,form_id,sub_indicator_10a,sub_indicator_10b,sub_indicator_10c,sub_indicator_10d,indicator_10_comments,score,visit_date,created_by,updated_by,created_at,updated_at) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [$id,$hf,$ssi,$fi,$s10a,$s10b,$s10c,$s10d,$i10,$sc,$vd,$cb,$ub,$ca,$ua]);
      DB::table('spars_storage_management_10')->where('upload_status','=',1)->update(['upload_status' => 2]);
    }


    $storage11 = StorageManagement11::where('upload_status', '=', 1)->get();
    foreach ($storage11 as $value ) {
      ...
      DB::connection('mysql_remote')->insert('insert into spars_storage_management_11 (id,health_facility_id,survey_summary_id,form_id,sub_indicator_11a,sub_indicator_11b,sub_indicator_11c,sub_indicator_11d,sub_indicator_11e,indicator_11_comments,score,visit_date,created_by,updated_by,created_at,updated_at) values (?,?,? ,? ,? ,? ,? ,? ,? ,? ,? ,? ,? ,? ,? ,?)', [$id,$hi,$ss,$fi,$s11a,$s11b,$s11c,$s11d,$s11e,$scm,$sc,$vd,$cb,$ub,$ca,$ua]);
      DB::table('spars_storage_management_11')->where('upload_status','=',1)->update(['upload_status' => 2]);
    }

    $storage12 = StorageManagement12::where('upload_status', '=', 1)->get();
    foreach ($storage12 as $value ) {
      ...
      DB::connection('mysql_remote')->insert('insert into spars_storage_management_12 (id,health_facility_id,survey_summary_id,form_id,sub_indicator_12a_main_store,sub_indicator_12b_main_store,sub_indicator_12c_main_store,sub_indicator_12d_main_store,sub_indicator_12e_main_store,sub_indicator_12a_lab_store,sub_indicator_12b_lab_store,sub_indicator_12c_lab_store,sub_indicator_12d_lab_store,sub_indicator_12e_lab_store,score,indicator_12_comments,visit_date,created_by,updated_by,created_at,updated_at) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [$id, $hi,$ss,$fi,$s12a,$s12b,$s12c,$s12d,$s12e,$sl12a,$sl12b,$sl12c,$sl12d,$sl12e,$sc,$i12cm,$vd,$cb,$ub,$ca,$ua]);

      DB::table('spars_storage_management_12')->where('upload_status','=',1)->update(['upload_status' => 2]);
    }

    $storage13 = StorageManagement13::where('upload_status', '=', 1)->get();
    ...
    foreach ($ordering15 as $value ) {
      ...
      DB::connection('mysql_remote')->insert('insert into spars_ordering_15 (id,health_facility_id,survey_summary_id,form_id,sub_indicator_15a,sub_indicator_15a_soh,sub_indicator_15a_issued,sub_indicator_15a_amc,sub_indicator_15b,sub_indicator_15c,score,indicator_15_comments,visit_date,created_by,updated_by,created_at,updated_at) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [$id,$hf,$si,$fi,$i15a,$i15asoh,$i15ai,$i15am,$i15b,$i15c,$sc,$i15cm,$vd,$cb,$ub,$ca,$ua]);

      DB::table('spars_ordering_15')->where('upload_status','=',1)->update(['upload_status' => 2]);
    }

    $ordering16 = Ordering16::where('upload_status', '=',1)->get();
    foreach ($ordering16 as $value ) {
      ...
      DB::connection('mysql_remote')->insert('insert into spars_ordering_16 (id,health_facility_id,survey_summary_id,form_id,sub_indicator_16a,sub_indicator_16b,sub_indicator_16c,sub_indicator_16d,sub_indicator_16e,sub_indicator_16f,indicator_16_comments,score,visit_date,created_by,updated_by,created_at,updated_at) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [$id,$hf,$si,$fi,$i16a,$i16b,$i16c,$i16d,$i16e,$i16f,$i16c,$sc,$vd,$cb,$ub,$ca,$ua]);

      DB::table('spars_ordering_16')->where('upload_status','=',1)->update(['upload_status' => 2]);
    }

    $ordering17 = Ordering17::where('upload_status', '=', 1)->get();
    foreach ($ordering17 as $value ) {
      ...
      DB::connection('mysql_remote')->insert('insert into spars_ordering_17 (id,health_facility_id,survey_summary_id,form_id,sub_indicator_17a,indicator_17_comments,score,visit_date,created_by,updated_by,created_at,updated_at) values (?,?,?,?,?,?,?,?,?,?,?,?)', [$id,$hf,$si,$fi,$i17a,$i17cm,$sc,$vd,$cb,$ub,$ca,$ua]);

      DB::table('spars_ordering_17')->where('upload_status','=',1)->update(['upload_status' => 2]);
    }

    $equipment18 = Equipment18::where('upload_status', '=', 1)->get();
    foreach ($equipment18 as $value ) {
      ...
      DB::connection('mysql_remote')->insert('insert into equipment_18 (id,health_facility_id,survey_summary_id,form_id,sub_indicator_18a,sub_indicator_18b,sub_indicator_18c,sub_indicator_18d,indicator_18_comments,score,visit_date,created_by,updated_by,created_at,updated_at) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [$id,$hf,$si,$fi,$i18a,$i18b,$i18c,$i18d,$i18cm,$sc,$vd,$cb,$ub,$ca,$ua]);

      DB::table('equipment_18')->where('upload_status','=',1)->update(['upload_status' => 2]);
    }

    $equipment19 = Equipment19::where('upload_status', '=', 1)->get();
    foreach ($equipment19 as $value ) {
      ...
      DB::connection('mysql_remote')->insert('insert into equipment_19 (id,health_facility_id,survey_summary_id,form_id,sub_indicator_19a,sub_indicator_19b,sub_indicator_19c,sub_indicator_19d,indicator_19_comments,score,visit_date,created_by,updated_by,created_at,updated_at) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [$id,$hf,$si,$fi,$i19a,$i19b,$i19c,$i19d,$i19cm,$sc,$vd,$cb,$ub,$ca,$ua]);

      DB::table('equipment_19')->where('upload_status','=',1)->update(['upload_status' => 2]);
    }

    $equipment20 = Equipment20::where('upload_status', '=', 1)->get();
    ...
    foreach ($infosys22 as $value ) {
      ...
      DB::connection('mysql_remote')->insert('insert into spars_info_system_22 (id,health_facility_id,survey_summary_id,form_id,sub_indicator_22a,sub_indicator_22b,sub_indicator_22c,sub_indicator_22d,sub_indicator_22e,sub_indicator_22f,sub_indicator_22g,sub_indicator_22h,sub_indicator_22i,sub_indicator_22j,sub_indicator_22k,sub_indicator_22l,sub_indicator_22m,sub_indicator_22n,indicator_22_comments,score,visit_date,created_by,updated_by,created_at,updated_at) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [$id,$hf,$si,$fi,$si22a,$si22b,$si22c,$si22d,$si22e,$si22f,$si22g,$si22h,$si22i,$si22j,$si22k,$si22l,$si22m,$si22n,$sicmt,$sc,$vd,$cb,$ub,$ca,$ua]);

      DB::table('spars_info_system_22')->where('upload_status','=',1)->update(['upload_status' => 2]);
    }


        $infosys23 = InformationSystem23::where('upload_status', '=', 1)->get();
        foreach ($infosys23 as $value ) {
          ...
          DB::connection('mysql_remote')->insert('insert into spars_info_system_23 (id,health_facility_id,survey_summary_id,form_id,sub_indicator_23a,sub_indicator_23b,indicator_23_comments,score,visit_date,created_by,updated_by,created_at,updated_at) values(?,?,?,?,?,?,?,?,?,?,?,?,?)', [$id,$hf,$si,$fi,$i23a,$i23b,$i23cm,$sc,$vd,$cb,$ub,$ca,$ua]);

          DB::table('spars_info_system_23')->where('upload_status','=',1)->update(['upload_status' => 2]);
        }

        $infosys24 = InformationSystem24::where('upload_status', '=', 1)->get();
        foreach ($infosys24 as $value ) {
          ...
          DB::connection('mysql_remote')->insert('insert into spars_info_system_24 (id,health_facility_id,survey_summary_id,form_id,sub_indicator_24a,sub_indicator_24b,sub_indicator_24c, indicator_24_comments,score,visit_date,created_by,updated_by,created_at,updated_at) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [$id,$hf,$si,$fi,$i24a,$i24b,$i24c,$i24cm,$sc,$vd,$cb,$ub,$ca,$ua]);

          DB::table('spars_info_system_24')->where('upload_status','=',1)->update(['upload_status' => 2]);
        }

        $infosys25 = InformationSystem25::where('upload_status', '=', 1)->get();
        ...
        foreach ($infosys26 as $value ) {
          ...
          DB::connection('mysql_remote')->insert('insert into spars_info_system_26 (id,health_facility_id,survey_summary_id,form_id,sub_indicator_26a,sub_indicator_26b,indicator_26_comments,score,visit_date,created_by,updated_by,created_at,updated_at) values(?,?,?,?,?,?,?,?,?,?,?,?,?)', [$id,$hf,$si,$fi,$si26a,$si26b,$si26cm,$sc,$vd,$cb,$ub,$ca,$ua]);

          DB::table('spars_info_system_26')->where('upload_status','=',1)->update(['upload_status' => 2]);
        }

        $infosys27 = InformationSystem27::where('upload_status', '=', 1)->get();
        foreach ($infosys27 as $value ) {
          ...
          DB::connection('mysql_remote')->insert('insert into spars_info_system_27 (id,health_facility_id,survey_summary_id,form_id,sub_indicator_27a,sub_indicator_27b,sub_indicator_27c,sub_indicator_27d,indicator_27_comments,score,visit_date,created_by,updated_by,created_at,updated_at) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', [$id,$hf,$si,$fi,$si27a,$si27b,$si27c,$si27d,$i27cm,$sc,$vd,$cb,$ub,$ca,$ua]);

          DB::table('spars_info_system_27')->where('upload_status','=',1)->update(['upload_status' => 2]);
        }
